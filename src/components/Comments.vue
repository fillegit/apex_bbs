          ...
          <v-container>
            ...
            <h3>投稿フォーム</h3>
              ...
              <select class="selectBox" v-model="inputPlaystyle">
                  <option disabled value="">プレイスタイルを選択してください</option>
                  <option
                    v-for="(v, i) in playstyles"
                    :key="i"
                    :value="v"
          ...
          <v-list-item-content>
            <v-list-item-subtitle class="text--primary subheading">
              <v-layout wrap class="contentLine">
                ...
                <v-flex><span class="contentTitle">最高到達ランク : </span>{{ comment.maxRank }}</v-flex> 
              </v-layout>
              <div class="contentLine"><span class="contentTitle">自分のキャラ : </span>{{ arrayToString(comment.myCharacter) }}</div>
              <div class="contentLine"><span class="contentTitle">欲しいキャラ : </span>{{ arrayToString(comment.seekingCharacter) }}</div>
              <!-- <v-layout wrap class="contentLine">
                <v-flex><span class="contentTitle">獲得バッジ : </span>{{ comment.badge }}</v-flex>
              </v-layout> -->
              <div class="contentLine"><span class="contentTitle">コメント : </span>{{ comment.comment }}</div>
              <div><span class="contentTitle">ID : </span>{{ comment.playerId }}</div>
...
export default {
  data() {
    return {
      ...
      badges: ["2000ハンマー", "2500ハンマー", "3000ハンマー", "4000ハンマー", "爪痕"],
    };
  },
  firestore() {
    return {
